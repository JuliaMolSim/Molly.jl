...
function Molly.AllegroPotential(path::AbstractString; T::Type=Float32)
    h5open(path, "r") do f
        ...
        env_p = Int(cfg["env_p"])
        rc = Float64(cfg["rc"])
        avg_nn = Float64(cfg["avg_nn"])
        rd(p) = reverse_dims(read(f[p]))
        layers = map(0:L-1) do li
            g = "layer$li"
            (env_W=rd("$g/env_W"), env_b=rd("$g/env_b"),
        ...
        weights = (emb_W1=rd("emb_W1"), emb_b1=rd("emb_b1"), emb_W2=rd("emb_W2"), emb_b2=rd("emb_b2"),
                   ...
                   layers=layers)
        model = build_allegro_model(; C=C, H=H, nb=nb, S=S, L=L, env_p=env_p, r_c=rc,
                                    avg_nn=avg_nn, weights=weights, T=T)
