...
It becomes available when [Lux.jl](https://github.com/LuxDL/Lux.jl) and [HDF5.jl](https://github.com/JuliaIO/HDF5.jl) are loaded; loading [Enzyme.jl](https://github.com/EnzymeAD/Enzyme.jl) in addition gives single-pass reverse-mode AD forces (otherwise forces fall back to finite differences).
The weights are read from an HDF5 file generated by `test/torchani_reference.py` (see `data/ani_reference/ani2x.h5`).
```julia
using Molly
using Lux, HDF5 # required to construct and run an ANIPotential
using Enzyme    # optional, for fast single-pass AD forces

data_dir = joinpath(dirname(pathof(Molly)), "..", "data")
pot = ANIPotential(joinpath(data_dir, "ani_reference", "ani2x.h5"))
