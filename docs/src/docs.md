...
sys = System(atoms=atoms, coords=coords, velocities=velocities, neighbor_finder=nf, pairwise_inters=pairwise_inters, specific_inter_lists=specific_inter_lists, box_size=box_size, loggers=Dict{Symbol,Any}())
```
...
V_Type = eltype(s.velocities)
sys.loggers = Dict(:velocity_autocorrelation => TimeCorrelationLogger(T, T, V, V, n_atoms, 1000))
...
t_range=(0:999)*u"ps"
plot(t_range,sys.loggers[:velocity_autocorrelation].normalized_correlation)
```
