# Shared benchmarking harness for the Allegro scripts: timing with repeats + a variance band,
# a run/version header, and JSON output. `include` this from the individual scripts.
# (Identical in spirit to benchmark/ani_bench_common.jl — the harness is potential-agnostic.)
#
#   using JSON3           # optional; JSON writing is skipped if it is not loaded
#   include("allegro_bench_common.jl")
#   r = bench(() -> allegro_total_energy(...))   # → (min, median, iqr, bytes) in ms
#
