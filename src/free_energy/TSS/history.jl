...
function TSSHistoryForgetting(; alpha::Real = 0.19,
    ...
    if !(isfinite(phi_value) && phi_value > 1)
...
function convert_tss_history_config(::Type{FT},
...
function TSSEpochHistory(config::TSSHistoryForgetting,
    ...
    return TSSEpochHistory{FT}(
       convert_tss_history_config(FT, config),
        [0, 1],
        TSSEpoch{FT}[],
    )
end

function ensure_tss_epoch_bounds!(history::TSSEpochHistory, t::Integer)
...
function tss_epoch_index!(history::TSSEpochHistory, t)
    t <= 0 && return 1
    t_int = ceil(Int, t)
   ensure_tss_epoch_bounds!(history, t_int)
    return searchsortedfirst(history.taus, t_int)
end

function tss_epoch_for_update!(history::TSSEpochHistory{FT},
                                t::Integer,
                                n_states::Integer;
                                n_adaptive_moments::Integer = 0) where {FT}
    epoch_index =tss_epoch_index!(history, t)
...
function drop_old_tss_epochs!(history::TSSEpochHistory, t::Integer)
    first_recent =tss_first_retained_epoch_index!(history, t)
    filter!(epoch -> epoch.index >= first_recent, history.epochs)
    return history
end

function tss_first_retained_epoch_index!(history::TSSEpochHistory, t::Integer)
    threshold = ceil(Int, history.config.alpha * t)
    return tss_epoch_index!(history, threshold)
end

function tss_retained_epoch_indices!(history::TSSEpochHistory, t::Integer)
    t > 0 || throw(ArgumentError("TSS jackknife requires a positive history time."))
   ensure_tss_epoch_bounds!(history, t)
    first_recent = max(2,tss_first_retained_epoch_index!(history, t))
    current =tss_epoch_index!(history, t)
    if current < first_recent
        throw(ArgumentError("TSS jackknife could not identify retained epochs at time $(t)."))
    end
    return collect(first_recent:current)
end

function tss_epoch_weights!(history::TSSEpochHistory{FT},
...
function aggregate_tss_history!(state)
   ...
   aggregate_tss_history_adaptive_moments!(state)
    return state
end

function tss_history_sample_count(history;
...
function aggregate_tss_history_free_energies(state;
                                              omit_epoch_index = nothing,
                                              epoch_indices = nothing)
    history = state.history
    if isnothing(history)
    ...
    total_count =tss_history_sample_count(history;
...
function update_tss_history!(state,
                              visited_local::Int,
                              log_den,
                              history_time::Int;
    ...
    if !isnothing(adaptive_values)
        moments =ensure_tss_epoch_adaptive_moments!(
            epoch,
            length(state.f),
            n_adaptive_moments,
        )
   ...
   drop_old_tss_epochs!(history, history_time)
    aggregate &&aggregate_tss_history!(state)
    return state
end
