...
function apply_constraints!(sys, constraint::SHAKE, old_coords, dt)
    ...
    while(!converged)
        for r in 1:length(constraint.is)
            ...
            if abs(ustrip(norm(s01) - constraint.d[r])) > 1e-10
        ...
        lengths = [norm(vector(sys.coords[constraint.is[r]], sys.coords[constraint.js[r]], sys.boundary)) - constraint.d[r] for r in 1:length(constraint.is)]
        
        if ustrip(maximum(abs.(lengths))) < 1e-10
            converged = true
        end
    end        
end
