...
function force_kernel!(forces::CuDeviceMatrix{T}, virial, coords_var, atoms_var, boundary, inters,
                       neighbors_i_var, neighbors_j_var, ::Val{F}, ::Val{M}) where {T, F, M}
...
function forces(s::System{D, true, T}, neighbors=nothing; n_threads::Integer=Threads.nthreads()) where {D, T}
    ...
    if length(pairwise_inters_nl) > 0 && length(neighbors.close.nbsi) > 0
        ...
        CUDA.@sync @cuda threads=256 blocks=1600 force_kernel!(
